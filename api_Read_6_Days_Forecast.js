...
function isCriticalWeather(weatherDescription) {
    const criticalWeatherConditions = [
        ...
        'Snow and Ice Pellets', 'Light Fog','Mostly Clear'
    ];

    return criticalWeatherConditions.includes(weatherDescription);
}

// Define a flag to track if critical weather for the fourth day was displayed
let fourthDayCriticalDisplayed = false;
...
function fetchAndCheckWeather() {
    ...
    fetch(apiUrl)
        ...
        .then(data => {
            ...
            // Get the timeline for the fourth, fifth, and sixth days
            ...
            // Display critical weather alerts for the fourth, fifth, and sixth days
            displayCriticalWeatherAlert(fourthDayTimeline, 'weather', fifthDayTimeline, sixthDayTimeline);
        })
        .catch(error => {
...
// Function to display critical weather alert in HTML
function displayCriticalWeatherAlert(dayTimeline, elementId, nextDayElementId1, nextDayElementId2) {
    if (dayTimeline) {
        const dayWeather = dayTimeline.values.weatherCodeFullDay;
        if (dayWeather in weatherDescriptions) {
            ...
            if (isCriticalWeather(weatherDescription)) {
                // Set the flag to true since critical weather for the fourth day was displayed
                fourthDayCriticalDisplayed = true;

                const criticalWeatherAlert = `
                    <p>${weatherDescription} on ${dayTimeline.startTime}.</p>
                `;
                document.getElementById(elementId).innerHTML = criticalWeatherAlert;

                // Check if the next day alerts should be displayed
                if (elementId === 'weather' && fourthDayCriticalDisplayed) {
                    // Display critical weather alerts for the next two days
                    displayCriticalWeatherAlert(nextDayElementId1, 'weather1');
                    displayCriticalWeatherAlert(nextDayElementId2, 'weather2');
                }
            }
        }
    }

    // If critical weather for the fourth day was not displayed and 'weather' is not called, display the message
    if (!fourthDayCriticalDisplayed && elementId === 'weather') {
        const notCriticalMessage = 'The weather in three days from today will not be critical';
        document.getElementById('weather3').innerHTML = notCriticalMessage;
    }
}
